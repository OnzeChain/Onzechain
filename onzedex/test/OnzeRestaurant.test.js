...
const OnzeRestaurant = artifacts.require('OnzeRestaurant');

contract('OnzeRestaurant', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.onze = await OnzeToken.new({ from: alice });
        // 10% reduction per 1000 blocks
        this.rest = await OnzeRestaurant.new(this.onze.address, '999894645034566400', { from: alice });
        this.onze.mint(alice, '1000', { from: alice });
        this.onze.mint(bob, '1000', { from: alice });
        this.onze.mint(carol, '1000', { from: alice });
    });

    it('should work properly', async () => {
        await this.onze.approve(this.rest.address, '1000', { from: alice });
        await this.onze.approve(this.rest.address, '1000', { from: bob });
        ...
        // OnzeBar get 200 more OXs from an external source.
        await this.onze.transfer(this.rest.address, '200', { from: carol });
        ...
        // Alice deposits 200 more OXs. But it's worth 10% less
        await this.rest.enter('200', { from: alice });
        assert.equal((await this.onze.balanceOf(alice)).valueOf(), '733');
        assert.equal((await this.onze.balanceOf(bob)).valueOf(), '900');
        ...
        assert.equal((await this.onze.balanceOf(alice)).valueOf(), '1092');
        assert.equal((await this.onze.balanceOf(bob)).valueOf(), '1107');
        assert.equal((await this.rest.userInfo(alice)).amount.valueOf(), '200');
